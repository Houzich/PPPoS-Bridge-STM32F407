...
static dhcp_entry_t entries[NUM_DHCP_ENTRY] =
{
	...
	{ {0}, {192, 168, 5, 2}, {255, 255, 255, 0}, 24 * 60 * 60 },
	{ {0}, {192, 168, 5, 3}, {255, 255, 255, 0}, 24 * 60 * 60 },
	{ {0}, {192, 168, 5, 4}, {255, 255, 255, 0}, 24 * 60 * 60 }
};

static dhcp_config_t dhcp_config =
{
	{192, 168, 0, 111}, 67, // server address, port
	{0, 0, 0, 0},     			// dns server
	"stm",                	// dns suffix
...
static void udp_recv_proc(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
	uint8_t *ptr;
	dhcp_entry_t *entry;
	struct pbuf *pp;
	...
	switch (dhcp_data.dp_options[2])
	{
		case DHCP_DISCOVER:
			...
			*(uint32_t *)dhcp_data.dp_yiaddr = *(uint32_t *)entry->addr;
			...
			udp_sendto(pcb, pp, IP_ADDR_BROADCAST, port);
			pbuf_free(pp);
			break;
